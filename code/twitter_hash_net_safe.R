...
for(u in seed.user:length(hash.users)) {
    ...
    paste("Processed user:", hash.users[u])
}
...
# hash.graph<-read.edgelist(paste(hashtag,"/",hashtag,".net",sep=""),format="pajek")
...
p<-ggplot(hash.cent,aes(x=bet,y=eig,label=V(hash.graph)$name,colour=res,
    ...
    Centrality")
pdf(paste(hashtag,"/","03_",hashtag,"_key_actor.pdf",sep=""))
...
# MAKE SURE TO CHANGE TO YOUR KEY
ic.apikey<-"your_api_key"
